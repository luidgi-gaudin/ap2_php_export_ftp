<div class="container mt-5">
    <div class="row align-items-center mb-4">
        <div class="col-lg-6">
            <h1 class="neo-title">Liste des antécédents</h1>
        </div>
        <div class="col-lg-6 text-lg-end add-btn-container">
            <a href="/antecedants/create" class="neo-btn neo-btn-primary">
                <i class="bi bi-plus-lg"></i> Ajouter un antécédent
            </a>
        </div>
    </div>
    ...
    <div class="neo-card">
        ...
        <div class="mb-4">
            <div class="neo-search-container">
                ...
                <input type="text" id="searchInput" class="neo-search-input" placeholder="Rechercher un antécédent...">
            </div>
        </div>

        <div class="table-responsive neo-table-container">
            <table class="neo-table" id="antecedentTable">
                <thead>
                <tr>
                    <th>Libellé</th>
                ...
                <tbody>
                <?php if(count($antecedents) > 0): ?>
                    <?php foreach($antecedents as $antecedent): ?>
                        <tr>
                            <td class="antecedent-libelle"><?= htmlspecialchars($antecedent->getLibelle()) ?></td>
                            <td class="text-end">
                                <div class="neo-btn-group justify-content-end">
                                    <a href="/antecedants/details/<?= $antecedent->getAntecedentId() ?>" class="neo-btn neo-btn-small neo-btn-info">
                                        <i class="bi bi-eye"></i>
                                    </a>
                                    <a href="/antecedants/edit/<?= $antecedent->getAntecedentId() ?>" class="neo-btn neo-btn-small neo-btn-info">
                                        <i class="bi bi-pencil-square"></i>
                                    </a>
                                    <a href="/antecedants/delete/<?= $antecedent->getAntecedentId() ?>"
                ...
                <?php else: ?>
                    <tr>
                        <td colspan="2" class="text-center neo-empty">Aucun antécédent trouvé</td>
                    </tr>
                <?php endif; ?>
                </tbody>
            </table>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const table = document.getElementById('antecedentTable');
        ...
        searchInput.addEventListener('input', function() {
            ...
            rows.forEach(row => {
                const libelle = row.querySelector('.antecedent-libelle');
                if (!libelle) return;
