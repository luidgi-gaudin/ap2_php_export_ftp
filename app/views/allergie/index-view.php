<div class="container mt-5">
    <div class="row align-items-center mb-4">
        <div class="col-lg-6">
            <h1 class="neo-title">Liste des allergies</h1>
        </div>
        <div class="col-lg-6 text-lg-end add-btn-container">
            <a href="/allergie/create" class="neo-btn neo-btn-primary">
                <i class="bi bi-plus-lg"></i> Ajouter une allergie
            </a>
        </div>
    </div>
    ...
    <div class="neo-card">
        ...
        <div class="mb-4">
            <div class="neo-search-container">
                ...
                <input type="text" id="searchInput" class="neo-search-input" placeholder="Rechercher une allergie...">
            </div>
        </div>

        <div class="table-responsive neo-table-container">
            <table class="neo-table" id="allergieTable">
                <thead>
                <tr>
                    <th>Libellé</th>
                ...
                <tbody>
                <?php
                $allergies = \App\Models\Allergie::getAll();
                if(count($allergies) > 0):
                    foreach($allergies as $allergie):
                        ?>
                        <tr>
                            <td class="allergie-libelle"><?= htmlspecialchars($allergie->getLibelle()) ?></td>
                            <td class="text-end">
                                <div class="neo-btn-group justify-content-end">
                                    <a href="/allergie/details/<?= $allergie->getAllergieId() ?>" class="neo-btn neo-btn-small neo-btn-info">
                                        <i class="bi bi-eye"></i>
                                    </a>
                                    <a href="/allergie/edit/<?= $allergie->getAllergieId() ?>" class="neo-btn neo-btn-small neo-btn-info">
                                        <i class="bi bi-pencil-square"></i>
                                    </a>
                                    <a href="/allergie/delete/<?= $allergie->getAllergieId() ?>"
                    ...
                    ?>
                    <tr>
                        <td colspan="2" class="text-center neo-empty">Aucune allergie trouvée</td>
                    </tr>
                <?php endif; ?>
                </tbody>
            </table>
        ...
        <div id="noResults" class="neo-no-results" style="display: none;">
            <i class="fas fa-search me-2"></i> Aucun résultat ne correspond à votre recherche
        </div>
    </div>
</div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const table = document.getElementById('allergieTable');
        ...
        searchInput.addEventListener('input', function() {
            ...
            rows.forEach(row => {
                const libelle = row.querySelector('.allergie-libelle');
                if (!libelle) return;
