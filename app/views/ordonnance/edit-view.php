<div class="container mt-5">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1 class="neo-title">Modifier une ordonnance</h1>
    ...
    <?php endif; ?>
    <div class="neo-card">
        <form method="post" action="/ordonnance/edit/<?= isset($ordonnance) ? $ordonnance->getOrdonnanceId() : '' ?>">
            <div class="form-group mb-3">
                <label for="posologie" class="form-label">Posologie :</label>
                <textarea name="posologie" id="posologie" class="form-control" required><?= isset($ordonnance) ? $ordonnance->getPosologie() : '' ?></textarea>
            </div>
            <div class="form-group mb-3">
                <label for="duree_traitement" class="form-label">Durée du traitement :</label>
                <input type="text" name="duree_traitement" id="duree_traitement" class="form-control" value="<?= isset($ordonnance) ? $ordonnance->getDureeTraitement() : '' ?>" required>
            </div>
            <div class="form-group mb-3">
                <label for="instructions" class="form-label">Instructions spécifiques :</label>
                <textarea name="instructions" id="instructions" class="form-control" required><?= isset($ordonnance) ? $ordonnance->getInstructionsSpecifique() : '' ?></textarea>
            </div>
            <div class="form-group mb-3 patient-search-container">
                <label for="patientSearch" class="form-label">Patient :</label>
                <input type="text" id="patientSearch" class="form-control" placeholder="Rechercher par nom, prénom ou n° secu" autocomplete="off" value="<?= isset($patient) ? $patient->getPrenom() . ' ' . $patient->getNom() . ' (' . $patient->getNumSecu() . ')' : '' ?>" disabled>
                <input type="hidden" name="patientId" id="patientId" value="<?= isset($ordonnance) ? $ordonnance->getPatientId() : '' ?>">
            ...
            <div id="medicament-rows" class="mb-3">
                <?php if(!empty($associatedMedicaments)): ?>
                    <?php foreach($associatedMedicaments as $i => $med): ?>
                        <div class="medication-row mb-2">
                            <input type="text" class="form-control medication-search" placeholder="Rechercher un médicament" autocomplete="off" value="<?= htmlspecialchars($med->getLibelle()) ?>">
                            <input type="hidden" name="medicaments[<?= $i ?>][id]" class="medicament-id" value="<?= $med->getMedicamentId() ?>">
                            <input type="number" name="medicaments[<?= $i ?>][quantite]" class="form-control medication-quantity" value="<?= $med->quantite ?>" min="1">
                ...
                <?php else: ?>
                    <div class="medication-row mb-2">
                        ...
                        <input type="hidden" name="medicaments[][id]" class="medicament-id">
                        <input type="number" name="medicaments[][quantite]" class="form-control medication-quantity" value="1" min="1">
                        <button type="button" class="neo-btn neo-btn-danger remove-medicament"><i class="bi bi-trash"></i></button>
                        <div class="medication-suggestions suggestions-box"></div>
                    </div>
                <?php endif; ?>
...
<script>
    ...
    function addMedicationRow(medData = {}) {
        ...
        <input type="hidden" name="medicaments[][id]" class="medicament-id" value="${medData.id ? medData.id : ''}">
        <input type="number" name="medicaments[][quantite]" class="form-control medication-quantity" value="${medData.quantite ? medData.quantite : 1}" min="1">
        <button type="button" class="neo-btn neo-btn-danger remove-medicament"><i class="bi bi-trash"></i></button>
        <div class="medication-suggestions suggestions-box"></div>
    `;
