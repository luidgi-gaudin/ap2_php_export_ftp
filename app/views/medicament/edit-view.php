<div class="container mt-5">
    <div class="row align-items-center mb-4">
        <div class="col-lg-6">
            <h1 class="neo-title">Modifier le médicament</h1>
        </div>
    ...
    <div class="neo-card">
        ...
        <form method="post" action="/medicament/edit/<?= $medicament->getMedicamentId() ?>">
            <div class="mb-3">
                <label for="libelle" class="form-label">Libellé :</label>
                <input type="text" name="libelle" id="libelle" class="form-control" value="<?= htmlspecialchars($medicament->getLibelle()) ?>" required>
            </div>
            <div class="mb-3">
                <label for="contr_indication" class="form-label">Contre-indication :</label>
                <textarea name="contr_indication" id="contr_indication" class="form-control" required><?= htmlspecialchars($medicament->getContrIndication()) ?></textarea>
            </div>

            <h2 class="neo-title mt-4" style="font-size: 1.25rem;">Allergies associées</h2>
            <div id="allergy-selects" class="mb-3">
                <?php
                $currentAllergies = $medicament->getAllergies();
                if(empty($currentAllergies)) {
                    $currentAllergies = [''];
                }
                foreach ($currentAllergies as $selectedAllergie):
                    $selectedId = is_object($selectedAllergie) ? $selectedAllergie->getAllergieId() : '';
                    ?>
                    <div class="allergy-select mb-2 d-flex align-items-center">
                        <select name="allergies[]" class="form-select me-2">
                            <option value="">-- Sélectionner une allergie --</option>
                            <?php foreach ($allergies as $allergie): ?>
                                <option value="<?= $allergie->getAllergieId() ?>" <?= ($allergie->getAllergieId() == $selectedId) ? 'selected' : '' ?>>
                                    <?= htmlspecialchars($allergie->getLibelle()) ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                        <button type="button" class="neo-btn neo-btn-danger remove-allergy">
...
<script>
    document.getElementById('add-allergy').addEventListener('click', function() {
        ...
        var removeBtn = newSelectDiv.querySelector('.remove-allergy');
        if (!removeBtn) {
            var btn = document.createElement("button");
            btn.type = "button";
            btn.className = "neo-btn neo-btn-danger remove-allergy";
            btn.innerHTML = '<i class="bi bi-trash"></i>';
            btn.addEventListener('click', function() {
                this.parentElement.remove();
            });
            newSelectDiv.appendChild(btn);
        }
        container.appendChild(newSelectDiv);
    });

    // Suppression d'une allergie associée
